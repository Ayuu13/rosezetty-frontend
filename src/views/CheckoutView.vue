						...
						<div class="col-lg-5">
							<div class="me-lg-5">
								<div class="card-body1">
									...
									<select
										v-model="item.jumlah"
										class="opsi-jumlah form-select me-4"
										style="width: 50px">
			...
			<div class="pembayaran">
				<div class="bayar1">
					...
					<p>
						...
						<strong
							>‼️Payment ke QRIS khusus nominal di bawah 300k. Mohon memilih metode pembayaran lain untuk
							nominal di atas 300k‼️</strong
						><br />
					</p>
				</div>
				<div class="bayar2">
					<div class="input-group input-group-outline my-3">
						...
						<select id="metodebayar" v-model="selectedPaymentMethod" class="form-control">
							<option v-for="metode in metodes" :key="metode.id" :value="metode.id">
								{{ metode.nama }}
							</option>
						</select>
					</div>
					<div v-if="selectedPaymentMethod">
						<div v-if="selectedPaymentMethodObject">
							...
							<img
								v-if="selectedPaymentMethodObject.gambar"
								:src="getVariantImageUrl(selectedPaymentMethodObject.gambar)"
								:alt="'Gambar ' + selectedPaymentMethodObject.nama" />
						</div>
					</div>
...
export default {
	...
	setup() {
		...
		const checkedCartItems = computed(() => cartStore.cart.filter((item, index) => cartStore.checkedRows[index]));

		const totalQuantity = computed(() => checkedCartItems.value.reduce((total, item) => total + item.jumlah, 0));
		...
		const fetchMetodes = async () => {
			try {
				const response = await axios.get('api/metode-bayar');
		...
		const checkout = async () => {
			try {
				...
				const response = await axios.post('api/pesanan/tambah', formData, {
					headers: {
