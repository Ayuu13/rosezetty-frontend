				...
				<tbody>
					<tr v-for="(pesanan, index) in pesananList" :key="index">
						...
						<td class="align-middle text-center">
							<span :class="badgeClass(pesanan.status ? pesanan.status.nama : '')">{{
								pesanan.status ? pesanan.status.nama : 'N/A'
							}}</span>
						</td>
						<td class="align-middle text-center">
							<span :class="badgeClass(pesanan.barang ? pesanan.barang.nama : '')">{{
								pesanan.barang ? pesanan.barang.nama : 'N/A'
							}}</span>
						</td>
						<td>
							<router-link :to="'/pesanan-detail/' + pesanan.id"> Detail </router-link>
						</td>
					</tr>
				</tbody>
			</table>
	...
	methods: {
		async fetchPesanan(token) {
			try {
				...
				const response = await axios.get(`api/riwayat-pesanan/${userId}`, {
					headers: {
		...
		async fetchAdditionalDetails() {
			try {
				...
				const [statusResponses, versiProdukResponses, barangResponses] = await Promise.all([
					axios.get('api/status-pesanan', { params: { ids: statusIds } }),
					axios.get('api/varian-produk', { params: { ids: versiProdukIds } }),
					axios.get('api/status-barang', { params: { ids: barangIds } }),
				]);
