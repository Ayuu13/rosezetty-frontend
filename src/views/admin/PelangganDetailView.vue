<template>
	<div class="products" v-if="user">
		...
		<div class="prod">
			...
			<p>
				Nomor Whatsapp: <a :href="'https://wa.me/' + user.nowa">{{ user.nowa }}</a>
			</p>
		</div>
		...
		<div v-else class="product-variants">
	...
	methods: {
		async fetchUserDetails(token) {
			...
			try {
				...
				try {
					const pesananResponse = await axios.get(`api/riwayat-pesanan/${userId}`, {
						headers: {
							Authorization: `Bearer ${token}`,
						},
					});
					...
					if (!this.pesanans.length) {
						this.errorMessage = 'Pelanggan belum melakukan pesanan';
					} else {
							...
							for (let detail of pesanan.pesanandetail) {
								try {
									if (detail.versiproduk_id) {
										const versiproResponse = await axios.get(`api/varian-detail/${detail.versiproduk_id}`);
									...
									if (error.response && error.response.status === 404) {
										console.warn(`Total pesanan untuk varian ID ${detail.versiproduk_id} tidak ditemukan.`);
									} else {
