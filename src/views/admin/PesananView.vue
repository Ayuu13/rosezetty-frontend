<template>
	<div class="main">
		...
		<input type="text" v-model="searchQuery" placeholder="Cari No Pesanan..." class="search" />
	</div>
			...
			<table id="dataTable" width="100%">
				...
				<tbody class="table2 text-center">
					<tr v-for="(pesanan, index) in filteredPesananByStatus(status.id)" :key="index">
						...
						<td>
							<router-link :to="'/admin/pesanan/' + pesanan.id"> Detail </router-link>
						</td>
					</tr>
				</tbody>
			</table>
	...
	methods: {
		...
		filteredPesananByStatus(statusId) {
			...
			return this.pesananData
				...
				.filter((pesanan) => pesanan.no_pesanan.toLowerCase().includes(this.searchQuery.toLowerCase()));
		},
	},
};
</script>
